...
links:
  ...
  - title: Newsletter
    ...
    - ## rss
        - Because we are moving away from [IFTTT](<IFTTT.md>), we will (eventually) need to store member RSS fields in Knack and import them into Jekyll as part of the build process. Really, this only makes sense if member data is imported as files, rather than directly into Jekyll.
...
rss:
  ...
  - https://yakcollective.substack.com/feed```
    ...
    - ## password
        ...
        - [Nathan Acks](<Nathan Acks.md>) Obviously, we __shouldn't__ port this into Jekyll during build.
    - ## bio_short
...
- Updated the [member schema]([Schema | member data](<Schema | member data.md>))
